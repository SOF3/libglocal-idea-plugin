...
public class LgcParser implements PsiParser, LightPsiParser {
  ...
  // T_FLAG*
  private static boolean arg_type_0(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "arg_type_0")) return false;
    while (true) {
      int c = current_position_(b);
      if (!consumeToken(b, T_FLAG)) break;
      if (!empty_element_parsed_guard_(b, "arg_type_0", c)) break;
    }
    return true;
  }
